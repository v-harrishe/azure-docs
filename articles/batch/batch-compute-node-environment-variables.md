...
The [Azure Batch service](https://www.azure.cn/home/features/batch/) sets the following environment variables on compute nodes. You can reference these environment variables in task command lines, and in the programs and scripts run by the command lines.
...
| AZ_BATCH_ACCOUNT_URL            | The URL of the Batch account. | All tasks. | `https://myaccount.chinanorth.batch.chinacloudapi.cn` |
...
| AZ_BATCH_NODE_IS_DEDICATED      | If `true`, the current node is a dedicated node. If `false`, it is a [low-priority(THIS FEATURE IS NOT AVAILABLE ON AZURE CHINA CLOUD) node](batch-low-pri-vms.md). | All tasks. | `true` |
...
<!-- Update_Description: new article about batch compute node environment variables -->
<!--NEW.date: 12/21/2020-->