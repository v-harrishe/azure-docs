...
Use Azure Batch to run large-scale parallel and high-performance computing (HPC(THIS FEATURE IS NOT AVAILABLE ON AZURE CHINA CLOUD)) batch jobs efficiently in Azure. This tutorial walks through a Python example of running a parallel workload using Batch. You learn a common Batch application workflow and how to interact programmatically with Batch and Storage resources. You learn how to:
...
:::image type="content" source="./media/tutorial-parallel-python/pool.png" alt-text="Pool heat map":::
...
To interact with a storage account, the app uses the [azure-storage-blob](https://pypi.python.org/pypi/azure-storage-blob) package to create a [BlockBlobService](https://docs.azure.cn/python/api/azure-storage-blob/azure.storage.blob.blockblobservice.blockblobservice) object.
...
The app creates a [BatchServiceClient](https://docs.azure.cn/python/api/azure.batch.batchserviceclient) object to create and manage pools, jobs, and tasks in the Batch service. The Batch client in the sample uses shared key authentication. Batch also supports authentication through [Azure Active Directory](batch-aad-auth.md), to authenticate individual users or an unattended application.
...
Next, the sample creates a pool of compute nodes in the Batch account with a call to `create_pool`. This defined function uses the Batch [PoolAddParameter](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.pooladdparameter) class to set the number of nodes, VM size, and a pool configuration. Here, a [VirtualMachineConfiguration](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.virtualmachineconfiguration) object specifies an [ImageReference](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.imagereference) to an Ubuntu Server 18.04 LTS image published in the Azure Marketplace. Batch supports a wide range of VM images in the Azure Marketplace, as well as custom VM images.
...
In addition to physical node properties, this pool configuration includes a [StartTask](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.starttask) object. The StartTask executes on each node as that node joins the pool, and each time a node is restarted. In this example, the StartTask runs Bash shell commands to install the ffmpeg package and dependencies on the nodes.

The [pool.add](https://docs.azure.cn/python/api/azure-batch/azure.batch.operations.pooloperations) method submits the pool to the Batch service.
...
A Batch job specifies a pool to run tasks on and optional settings such as a priority and schedule for the work. The sample creates a job with a call to `create_job`. This defined function uses the [JobAddParameter](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.jobaddparameter) class to create a job on your pool. The [job.add](https://docs.azure.cn/python/api/azure-batch/azure.batch.operations.joboperations) method submits the pool to the Batch service. Initially the job has no tasks.
...
The app creates tasks in the job with a call to `add_tasks`. This defined function creates a list of task objects using the [TaskAddParameter](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.taskaddparameter) class. Each task runs ffmpeg to process an input `resource_files` object using a `command_line` parameter. ffmpeg was previously installed on each node when the pool was created. Here, the command line runs ffmpeg to convert each input MP4 (video) file to an MP3 (audio) file.

The sample creates an [OutputFile](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.outputfile) object for the MP3 file after running the command line. Each task's output files (one, in this case) are uploaded to a container in the linked storage account, using the task's `output_files` property.

Then, the app adds tasks to the job with the [task.add_collection](https://docs.azure.cn/python/api/azure-batch/azure.batch.operations.taskoperations) method, which queues them to run on the compute nodes. 
...
There are many approaches to monitoring task execution. The `wait_for_tasks_to_complete` function in this example uses the [TaskState](https://docs.azure.cn/python/api/azure-batch/azure.batch.models.taskstate) object to monitor tasks for a certain state, in this case the completed state, within a time limit.
