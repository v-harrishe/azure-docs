...
1. The security principal's identity is authenticated and an OAuth 2.0 token is returned.  The resource name to request a token is `https://login.partner.microsoftonline.cn/{tenantID}` where `{tenantID}` matches the Azure Active Directory tenant ID to which the service principal belongs.
...
<!-- Update_Description: new article about concept enable rbac -->
<!--NEW.date: 12/21/2020-->