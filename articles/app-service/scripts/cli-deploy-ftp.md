...
This sample script creates an app in App Service with its related resources, and then deploys a static HTML page using FTP. For FTP upload, the script uses [cURL](https://en.wikipedia.org (THIS WEB SITE IS NOT AVAILABLE ON AZURE CHINA CLOUD) /wiki/CURL) as an example. You can use whatever FTP tool to upload your files.
...
| [`az webapp deployment list-publishing-profiles`](https://docs.azure.cn/cli/webapp/deployment#az_webapp_deployment_list_publishing_profiles) | Get the details for available app deployment profiles. |
...
<!-- Update_Description: new article about cli deploy ftp -->
<!--NEW.date: 12/21/2020-->