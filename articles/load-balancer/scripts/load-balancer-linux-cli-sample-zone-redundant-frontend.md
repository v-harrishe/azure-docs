...
| [az network lb rule create](https://docs.azure.cn/cli/network/lb/rule#az_network_lb_rule_create) | Creates a load balancer rule. In this sample, a rule is created for port 80. As HTTP traffic arrives at the load balancer, it is routed to port 80 of the VMs included in a load-balanced set. |
...
<!-- Update_Description: new article about load balancer linux cli sample zone redundant frontend -->
<!--NEW.date: 12/21/2020-->